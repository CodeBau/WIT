...
LRESULT CALLBACK ServerLogWin(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT: {
        ...
        DrawText(ps.hdc, L"Podaj has這 do serwera:", -1, &rc, DT_SINGLELINE | DT_CENTER | DT_VCENTER);

        SetRect(&rc, 100, 149, 110 + button_size_x, 149 + button_size_y);
        DrawText(ps.hdc, L"Zapami皻aj dane logowania", -1, &rc, DT_SINGLELINE  | DT_VCENTER);
...
LRESULT CALLBACK ServerLogWinButton(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_COMMAND:
    {
        ...
        case 2:
        {
            ::MessageBox(hwnd_ServerLogWin, L"UTW紑Z KONTO", L"", MB_OK);
            flag_LogWin = 3;
            InvalidateRect(GetParent(hWnd), NULL, FALSE);
        }break;
        ...
        case 3:
        {
            ::MessageBox(hwnd_ServerLogWin, L"PRZYPOMNIJ HASΜ", L"", MB_OK);
            flag_LogWin = 4;
            InvalidateRect(GetParent(hWnd), NULL, FALSE);
        }break;
        }// switch (wParam)
        ...
        switch (flag_LogWin)
        {
            case 2:
            {
                switch (wParam)
                {
                    ...
                    case 2:
                    {
                        ::MessageBox(hwnd_LogWin, L"UTW紑Z KONTO", L"", MB_OK);
                        flag_LogWin = 3;
                        InvalidateRect(GetParent(hWnd), NULL, FALSE);
                    }break;
                    ...
                    case 3:
                    {
                        ::MessageBox(hwnd_LogWin, L"PRZYPOMNIJ HASΜ", L"", MB_OK);
                        flag_LogWin = 4;
                        InvalidateRect(GetParent(hWnd), NULL, FALSE);
                    }break;
                }// switch (wParam)
