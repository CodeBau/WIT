...
//----- Child Controls -----
	...
	SendMessage(hwnd_LogWin_EditOrganization, WM_SETFONT, WPARAM(m_font), TRUE);
	SendMessage(hwnd_LogWin_EditOrganization, EM_SETLIMITTEXT, EditOrganization_maximum_length, 0);

	hwnd_LogWin_CheckBoxLoginData = CreateWindowEx(0, L"Button", L"Zapamiêtaj dane logowania", WS_CHILD | WS_VISIBLE | BS_CHECKBOX | WS_TABSTOP, ((windx - 210) / 2), 175, 210, 25, hwnd_LogWin, (HMENU)ChildId::LoginCheckBox, hInstance, nullptr);
	...
	hwnd_LogWin_CheckBoxAutoLogin = CreateWindowEx(0, L"Button", L"Autologowanie", WS_CHILD | WS_VISIBLE | BS_CHECKBOX | WS_TABSTOP, ((windx - 210) / 2), 200, 210, 25, hwnd_LogWin, (HMENU)ChildId::AutoLoginCheckBox, hInstance, nullptr);
...
// ------ Buttons --------
	...
	m_centralButton.create((windx - button_size_x) / 2, (windy - 6 * window_grid), button_size_x, button_size_y,L"B_CENTRAL", hwnd(), (HMENU)ChildId::CenterButton);
	m_leftButton.create(15,  340, button_size_x, button_size_y, L"B_LEFT",  hwnd_LogWin, (HMENU)ChildId::LeftButton);
	m_rightButton.create(130 ,340, button_size_x, button_size_y, L"B_RIGHT", hwnd_LogWin, (HMENU)ChildId::RightButton);
...
void LogWindow::OnCommand_Login(WPARAM wParam, LPARAM lParam) {
	auto id = static_cast<ChildId>(LOWORD(wParam));
...
void LogWindow::OnCommand_Registration(WPARAM wParam, LPARAM lParam) {
}

void LogWindow::OnCommand_PasswordReminder(WPARAM wParam, LPARAM lParam) {
}
...
void LogWindow::OnPaint(PAINTSTRUCT& ps) {
	...
	switch (m_mode)
	{
		case Mode::Login:
		{
			...
			// HBRUSH hBrush = ::CreateSolidBrush(RGB(0, 72, 0)); // don't create it every single time
